...
sns_topic_arn = 'arn:aws:sns:us-east-1:528097070592:UserUploadCompleteTopic'
...
# Send SNS notification when a record is uploaded/changed
def send_sns_alert(operation, message, item):
    try:
        sns_message = {
            'operation': operation,
            'message': message,
        ...
        sns_client.publish(
            ...
            Subject=f'Alert: {operation} operation on DynamoDB Table'
        )
        print(f'Successfully sent SNS message for {operation}')
...
def save_employee(request_body):
    try:
        ...
        send_sns_alert('SAVE', 'A new employee was added.', request_body)  # Send SNS alert on save
        body = {
            'Operation': 'SAVE',
            'Message': 'SUCCESS',
            'Item': request_body
...
def modify_employee(employee_id, update_key, update_value):
    try:
        ...
        send_sns_alert('UPDATE', f'Employee {employee_id} updated.', updated_item)  # Send SNS alert on update
        body = {
            'Operation': 'UPDATE',
            'Message': 'SUCCESS',
...
def delete_employee(employee_id):
    try:
        ...
        send_sns_alert('DELETE', f'Employee {employee_id} deleted.', deleted_item)  # Send SNS alert on delete
        body = {
            'Operation': 'DELETE',
            'Message': 'SUCCESS',
            'Item': response
